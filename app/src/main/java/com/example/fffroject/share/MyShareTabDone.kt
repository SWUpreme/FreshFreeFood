...
class MyShareTabDone : Fragment() {
    // 바인딩 객체
    lateinit var binding: FragmentMyShareTabDoneBinding
    // Data에 있는 PostAll
    ...
    // 나눔 완료 조건문
    lateinit var whereInParams: ArrayList<String>
    ...
    override fun onCreateView(
        ...
        whereInParams = arrayListOf()                   // 파이어베이스 나눔 완료 조건문 조건들
        ...
        // 레이아웃 매니저 등록
        recyclerviewMyShare.layoutManager = LinearLayoutManager(activity)
        // 리사이클러 뷰 어댑터 등록
    ...
    inner class MyShareViewAdapter(): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            if(fridgeToss==true){
                ...
                region.setTextColor(ContextCompat.getColor(requireActivity().applicationContext, R.color.white))
                location.setTextColor(ContextCompat.getColor(requireActivity().applicationContext, R.color.white))
            }else{
                ...
                region.setTextColor(
                    ContextCompat.getColor(requireActivity().applicationContext,
                    R.color.blueblack
                ))
            ...
            if(status.equals("shareDone")||status.equals("pointDone")){
                ...
                title.setTextColor(ContextCompat.getColor(requireActivity().applicationContext, R.color.deep_gray))
            }else{
                // 진행중인 거래라면
    ...
    private fun loadData() {
        if (user != null) {
            // db 조회 시 상태값 조건문의 조건
            whereInParams.addAll(listOf("shareDone", "pointDone"))
            // db 조회
            db?.collection("post")
                ?.whereEqualTo("writer", user?.uid.toString())
                ?.whereIn("status", whereInParams)
